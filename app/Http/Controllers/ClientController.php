...
class ClientController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        if(\Auth::user()->can('create client'))
        {
            ...
            if($request->ajax)
            {
                return view('clients.createAjax', compact('access'));
            }
            else
            {
                $customFields = CustomField::where('module', '=', 'client')->get();

                return view('clients.create', compact('customFields','access'));
            }
        }
        else
        {
                ...
                if(\Auth::user()->can('create client'))
                {
                    ...
                    if($total_client < $plan->max_clients || $plan->max_clients == -1)
                    {
                       ...
                       $acount=Acount::find($request->acount_id);
                        ...
                        $client->acount_id = $request->acount_id;
                        $client->acount = $acount->name;
