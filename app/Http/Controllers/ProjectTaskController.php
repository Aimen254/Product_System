...
class ProjectTaskController extends Controller
{
    ...
    {
          $proj = Project::where('created_by',\Auth::user()->creatorId())->pluck('project_name')->toArray();
        if($view == 'list'){
            return view('project_task.taskboard', compact('view','proj'));
          }else{
              $tasks = ProjectTask::where('created_by',\Auth::user()->creatorId())->get();
            return view('project_task.grid', compact('tasks','view','proj'));
          }
          return redirect()->back()->with('error', __('Permission Denied.'));
