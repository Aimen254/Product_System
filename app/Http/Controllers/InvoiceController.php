...
class InvoiceController extends Controller
{
    ...
    { 
       $user = \Auth::user();
        if(\Auth::user()->can('create invoice'))
        {
            ...
            $store =$user->stores->pluck('name', 'id');
            $store->prepend('--', '');

            return view('invoice.create', compact('customers', 'invoice_number', 'product_services', 'category', 'customFields', 'customerId','store'));
        }
        else
        {
    ...
    {
        $productServices = ProductService::find($request->product_id);
        ...
        $alllcost = $boxcost + $labourcost;
        $data['allcost'] = $boxcost + $labourcost;
        $data['totalcost'] = ($alllcost * $boxno);
        $data['store'] =  $productServices->stores;
        // $quantity            = 1;
    ...
    {
        $data['storeinfo']     = $storeinfo = Store::find($request->store_id);
        $data['storenam']     = $storeinfo->budget;
        $data['blnce']     = $storeinfo->blnce;
        return json_encode($data);
    }
    ...
    public function store(Request $request)
    {
        ...
        if(\Auth::user()->can('create invoice'))
        {
            ...
            for($i = 0; $i < count($products); $i++)
            {
                ...
                $invoiceProduct->store_id  = $products[$i]['store'];
                $invoiceProduct->storebudget  = $request->budget;
                $invoiceProduct->quantity    = $products[$i]['quantity'];
                $invoiceProduct->price       = $products[$i]['price'];
                $invoiceProduct->description = $products[$i]['description'];
                $invoiceProduct->thpl = $products[$i]['thpl'];
                $invoiceProduct->boxcost = $products[$i]['boxcost'];
                $invoiceProduct->boxno = $products[$i]['boxno'];
                $invoiceProduct->save();
                $expense = new StoreExpense();
                $expense->store_id = $products[$i]['store'];
                $expense->product_id = $products[$i]['item'];
                $expense->invoice_id =$invoice;
                $expense->amount = $products[$i]['expense'];
                $expense->save();
                $store = Store::find($products[$i]['store']);
                $store->expense = $products[$i]['expense'];
                $store->blnce =  $products[$i]['budget'];;
                $store->update();
                $prod = ProductService::find($products[$i]['item']);
                $prod->invoice = "invoiced";
                $prod->update();
    ...
    public function edit($ids)
    {
        ...
        if(\Auth::user()->can('edit invoice'))
        {
            ...
            return view('invoice.edit', compact('customers', 'product_services', 'invoice', 'invoice_number', 'category', 'customFields','store'));
        }
        else
        {
            return response()->json(['error' => __('Permission denied.')], 401);
...
//        $company_logo = Utility::getValByName('company_logo');
    ...
    public function invoice($invoice_id)
    {
        ...
        foreach($invoice->items as $product)
        {
            ...
            $item->thpl = $product->thpl;
            $item->boxcost = $product->boxcost;
            $item->boxno = $product->boxno;

            $totalQuantity += $item->quantity;
            $totalQuantity += $item->boxno;
            $totalRate     += $item->price;
            $totalRate     += $item->boxcost;
...
//        $company_logo = Utility::getValByName('company_logo');
    ...
    public function getstore(Request $request)
    {
        ...
        $data['storenam']     = $storeinfo->budget;
        $data['blnce']     = $storeinfo->blnce;
        $items = InvoiceProduct::where('invoice_id', $request->invoicee_id)->where('product_id', $request->product_id)->first();
        $totalamount = ($items->price * $items->quantity) + ($items->boxcost * $items->boxno);
