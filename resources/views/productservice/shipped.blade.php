...
@section('breadcrumb')
 <li class="breadcrumb-item"><a href="{{route('dashboard')}}">{{__('Dashboard')}}</a></li>
    ...
    <li class="breadcrumb-item">{{__('Product Shipped')}}</li>
@endsection
@section('action-btn')
    <div class="float-end">
                            ...
                            @foreach ($productServices as $productService)
                                <tr class="font-style">
                                    ...
                                    <td>{{ $productService->delivery }}</td>
                                        ...
                                        @if($productService->invoice == null)
                                            <td>Not_Invoiced</td>
                                        @else
                                         <td> {{ $productService->invoice }}</td>   
                                        @endif
                                    ...
                                    <td>
                                        <a href="#" data-size="md" data-url="{{ route('prod.users.edit',$productService->id) }}" data-ajax-popup="true" data-bs-toggle="tooltip" title="{{__('Add User')}}" class="btn btn-sm btn-primary">
                                            <i class="ti ti-plus"></i>
                                        </a>
                                    </td>
                                        ...
                                        <td class="Action" >
                                            ...
                                            @can('edit product & service')
                                                  <div class="action-btn">
                                                   <a href="{{ route('label.edit',$productService->id) }}" class="btn btn-sm btn-primary  " title="{{__('Purchase Status')}}"  data-title="{{__('Edit Status')}}" style="margin-left: 60px; ">
                                                   <span class="btn-inner--icon">{{__('Labels')}}</span>
                                                    </a>
                                                    <!-- <a href="#" class="btn btn-sm btn-primary  " data-url="{{ route('label.edit',$productService->id) }}" data-ajax-popup="true"  data-size="md" data-bs-toggle="tooltip" title="{{__('Purchase Status')}}"  data-title="{{__('Purchase Status')}}" style="margin-right: 10px;">
                                                   <span class="btn-inner--icon">{{__('Labels')}}</span>
                                                    </a> -->
                                                </div>
                                            @endcan
